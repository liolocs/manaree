...
if ( ! class_exists( 'YITH_WFBT_Multivendor' ) ) {
	...
	class YITH_WFBT_Multivendor {
		...
		public function redirect_data_tab() {
			...
			if ( isset( $_GET['page'] ) && $_GET['page'] === $page && ! isset( $_GET['tab'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
		 ...
		 * @access public
		 *
		 * @param string $parent_page Default parent page value.
		 *
		 * @return string
		 * @since  1.0.0
		 */
		 ...
		 * @access public
		 *
		 * @param string $capability Panel default capability.
		 * @return string
		 * @since  1.0.0
		 */
		public function admin_vendor_register_panel_capabilities( $capability ) {
		...
		public function is_vendor_product( $product_id ) {
			return empty( $this->vendor_products ) || in_array( $product_id, $this->vendor_products, true );
		}

		/**
		 * Add plugin screen id to main WC array
		 *
		 * @param  array $ids An array of screen ids.
		 * @return array
		 */
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $where Data table where conditions.
		 * @return string
		 */
 ...
 * @return \YITH_WFBT_Multivendor
 */
function YITH_WFBT_Multivendor() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
