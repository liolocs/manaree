	...
	class YITH_WFBT {
        ...
        public function declare_wc_features_support() {
            if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
                $init = defined( 'YITH_WFBT_INIT' ) ? YITH_WFBT_INIT : false;
		...
		public function is_admin() {
			$context_check = isset( $_REQUEST['context'] ) && 'frontend' === sanitize_text_field( wp_unslash( $_REQUEST['context'] ) ); // phpcs:ignore WordPress.Security.NonceVerification
			$is_admin      = is_admin() && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX && $context_check );

			/**
			 * APPLY_FILTERS: yith_wfbt_check_is_admin
			 *
		...
		public function add_group_to_cart() {
			...
			if ( ! ( isset( $_REQUEST['action'] ) && sanitize_text_field( wp_unslash( $_REQUEST['action'] ) ) === $this->actionadd && ( isset( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce'] ) ), $this->actionadd ) ) ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
				return;
			}
			...
			$message        = array();
			$offered        = ( isset( $_POST['offeringID'] ) && is_array( $_POST['offeringID'] ) ) ? array_map( 'absint', $_POST['offeringID'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput

			if ( empty( $offered ) ) {
				return;
			}

			$main_product = isset( $_POST['yith-wfbt-main-product'] ) ? intval( $_POST['yith-wfbt-main-product'] ) : absint( $offered[0] );
			...
			/**
			 * DO_ACTION: yith_wfbt_group_added_to_cart
			 *
			 * Allows to trigger some action when the group of products are added to the cart.
			 *
			 * @param array $products_added Products added to the cart.
			 * @param int   $main_product   Main product ID.
			 * @param array $offered        Products offered in the group.
			 */
			...
			if ( 'yes' === get_option( 'yith-wfbt-redirect-checkout', 'no' ) ) {
				...
				/**
				 * APPLY_FILTERS: yith_wfbt_redirect_after_add_url
				 *
				 * Filter the redirect URL when adding products to cart.
				 *
				 * @param string The URL.
				 *
				 * @return string
				 */
				$url = apply_filters( 'yith_wfbt_redirect_after_add_url', remove_query_arg( array( 'action', '_wpnonce' ) ) );
			}
		 ...
		 * @param integer $object_id Meta object ID.
		 * @param string  $meta_key Meta key.
		 * @param boolean $single True to get single, false otherwise.
		 * @return mixed
		 */
		 ...
		 *
		 * @since  1.3.0
		 * @param null|boolean $check Filter value.
		 * @param integer      $object_id Meta object ID.
		 * @param string       $meta_key Meta key.
		 * @param mixed        $value Meta value.
		 * @return mixed
		 */
		public function update_meta( $check, $object_id, $meta_key, $value ) {
		 ...
		 *
		 * @since  1.4.0
		 * @param array $columns An array of CSV columns.
		 * @return array
		 */
		 ...
		 *
		 * @since  1.4.0
		 * @param string     $value Column default value.
		...
		public function export_column_value( $value, $product, $column_id ) {
			...
			if ( ! empty( $data['products'] ) ) {
				$data['products'] = $this->prepare_products_for_export( $data['products'] );
			}
		 ...
		 *
		 * @param array                   $parsed_data An array of parsed data.
		 * @param WC_Product_CSV_Importer $importer WC_Product_CSV_Importer instance.
		 * @return array
		 */
		public function import_parse_plugin_data( $parsed_data, $importer ) {
			...
			if ( $parsed_data['frequently_bought_together'] !== $data ) {
				if ( ! is_array( $data['products'] ) ) {
					$data['products'] = array();
				}
			} else {
				$data = array(
					'products' => explode( ',', wc_clean( $parsed_data['frequently_bought_together'] ) ),
				);
			}
		...
		public function register_plugin_for_activation() {
			if ( ! class_exists( 'YIT_Plugin_Licence' ) ) {
				require_once YITH_WFBT_DIR . 'plugin-fw/lib/yit-plugin-licence.php';
			}
 ...
 * @return YITH_WFBT
 */
function YITH_WFBT() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
