...
class Iconic_WDS_Order {
	...
	public static function render_shop_order_columns( $column, $order ) {
		...
		switch ( $column ) {
			case 'jckwds_delivery':
				$jckwds->display_date_and_timeslot( $current_order, false, false, Iconic_WDS_Helpers::date_format() );
				break;
		}
	}
	...
	public static function get_shipping_method_id( $order ) {
		...
		$method = Iconic_WDS_Helpers::get_shipping_method_from_instance_id( $instance_id );
	...
	public static function get_order_date_time( $order ) {
		...
		$time            = Iconic_WDS_Helpers::get_order_meta( $order, $iconic_wds->timeslot_meta_key );
		$timestamp       = Iconic_WDS_Helpers::get_order_meta( $order, $iconic_wds->timestamp_meta_key );
		$ymd             = Iconic_WDS_Helpers::get_order_meta( $order, $iconic_wds->date_meta_key . '_ymd' );
		$shipping_method = Iconic_WDS_Helpers::get_order_meta( $order, $iconic_wds->shipping_method_meta_key );
		$timeslot_id     = Iconic_WDS_Helpers::get_order_meta( $order, '_jckwds_timeslot_id' );
		$override_rules  = Iconic_WDS_Helpers::get_order_meta( $order, '_jckwds_override_rules' );
		...
		if ( ! $has_meta ) {
			 ...
			 * @since 1.19.0
			 */
			return apply_filters( 'iconic_wds_delivery_slot_data', false, $order );
		}
		 ...
		 * @since 1.19.0
		 */
		return apply_filters( 'iconic_wds_delivery_slot_data', $meta, $order );
	}
	 ...
	 */
	public static function update_order_meta( $order_id, $data = array() ) {
		global $iconic_wds, $iconic_wds_dates;

		$timeslot = false;

		$posted_date = Iconic_WDS_Helpers::get_filtered_input( 'jckwds-delivery-date' );
		if ( false === $posted_date && isset( $data['jckwds-delivery-date'] ) ) {
			$posted_date = $data['jckwds-delivery-date'];
		}

		$posted_date_ymd = Iconic_WDS_Helpers::get_filtered_input( 'jckwds-delivery-date-ymd' );
		if ( false === $posted_date_ymd && isset( $data['jckwds-delivery-date-ymd'] ) ) {
			$posted_date_ymd = $data['jckwds-delivery-date-ymd'];
		}

		$posted_time = Iconic_WDS_Helpers::get_filtered_input( 'jckwds-delivery-time' );
		if ( false === $posted_time && isset( $data['jckwds-delivery-time'] ) ) {
			$posted_time = $data['jckwds-delivery-time'];
		}

		$posted_shipping_method = Iconic_WDS_Helpers::get_filtered_input( 'shipping_method', 'array' );
		if ( false === $posted_shipping_method && isset( $data['shipping_method'] ) ) {
			$posted_shipping_method = $data['shipping_method'];
		}

		$date_changed = Iconic_WDS_Helpers::get_filtered_input( 'jckwds-date-changed', 'int' );
		...
		if ( ! empty( $posted_date_ymd ) ) {
			...
			if ( $iconic_wds->has_reservation() && ! is_admin() ) {
				$iconic_wds->update_reservation( $slot_id, $order_id );
			} else {
				$data = array(
					'datetimeid' => $slot_id,
					'date'       => Iconic_WDS_Helpers::convert_date_for_database( $posted_date_ymd ),
					'order_id'   => $order_id,
					'processed'  => 1,
				);
			...
			$iconic_wds->add_timestamp_order_meta( $posted_date_ymd, $timeslot, $order_id );
		}
	 ...
	 *
	 * This is an Alias of Iconic_WDS_Order::get_order_date_time. To avoid
