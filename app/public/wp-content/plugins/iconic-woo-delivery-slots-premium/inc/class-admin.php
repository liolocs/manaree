...
class Iconic_WDS_Admin {
	...
	public static function admin_scripts() {
		...
		$script_vars = array(
			'bookable_dates' => $iconic_wds_dates->get_upcoming_bookable_dates( Iconic_WDS_Helpers::date_format() ),
	...
	public static function display_timeslot_line_item( $order_id ) {
		...
		if ( $date ) {
			$updated_class = '';
			$sub_orders    = Iconic_WDS_Edit_Timeslots::get_child_orders( $order );
			$parent_order  = Iconic_WDS_Edit_Timeslots::is_sub_order( $order );
	...
	public static function save_fee_to_order( $fees, $order ) {
		...
		$existing_item = Iconic_WDS_Helpers::get_delivery_fees_line_item( $order );
		$fee_line_item = false;
		...
		if ( ! $existing_item ) {
			...
			$fee_line_item->set_name( $iconic_wds->get_fee_name() );
		} else {
	...
	public static function get_order_id_from_param() {
		$is_hpos = Iconic_WDS_Helpers::is_cot_enabled();
