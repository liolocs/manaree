...
class Iconic_WDS_Dates {
	 ...
	 *
	 * @param array $args {
	 *     Optional. Array of argument that can be used to initialize the Iconic_WDS_Dates object.
	 *
	...
	public function __construct( $args = array() ) {
		...
		$this->cart = new Iconic_WDS_Cart( array() );
		...
		if ( ! empty( $args['order_id'] ) ) {
			...
			$this->cart            = new Iconic_WDS_Cart( $args['cart_products'] );
	...
	public function prepare_from_order_id( $order_id ) {
		...
		$this->cart = new Iconic_WDS_Cart( $products );
	...
	public function get_upcoming_bookable_dates( $format = 'array', $ignore_slots = false, $no_cache = false ) {
		...
		$conflict        = Iconic_WDS_Override_Settings::get_conflict_if_exists( $this->cart->get_products_ids() );
		...
		foreach ( $date_range as $timestamp ) {
			...
			if ( 'array' === $format ) {
				$available_dates[] = array(
					...
					'admin_formatted'  => date_i18n( Iconic_WDS_Helpers::date_format(), $timestamp ),
					'timestamp'        => $timestamp,
					'ymd'              => $ymd,
	...
	public function get_minmax_delivery_date( $type = 'min' ) {
		...
		$next_timestamp = $this->get_next_allowed_timestamp( Iconic_WDS_Helpers::get_current_timestamp(), $days, $max_days );
	...
	public function get_lead_time() {
		...
		foreach ( $product_ids as $product_id ) {
			...
			$product_lead_time = Iconic_WDS_Override_Settings::get_lead_time_for_product( $parent_product_id );
	...
	public function get_specific_delivery_dates( $skip_min_max_check = false ) {
		...
		$overridden_days = Iconic_WDS_Override_Settings::get_product_specific_days_setting( $this->cart->get_products_ids() );
		...
		if ( ! empty( $holidays ) ) {
			...
			foreach ( $holidays as $holiday ) {
				...
				if ( ! empty( $holiday_to ) ) {
					...
					$range = $this->create_timestamp_range( $holiday_from_object->getTimestamp(), $holiday_to_object->getTimestamp() );
				}
	...
	public function get_next_day_date( $format = 'timestamp' ) {
		...
		$next_day_timestamp = $this->get_next_allowed_timestamp( Iconic_WDS_Helpers::get_current_timestamp(), $min_days > 1 ? $min_days : 1 );
	...
	public function get_same_day_date( $format = 'timestamp' ) {
		$same_day_timestamp = $this->get_next_allowed_timestamp( Iconic_WDS_Helpers::get_current_timestamp() );
	...
	public function is_day_allowed( $timestamp, $calculation = false ) {
		...
		$allowed         = false;
		$is_same_day     = Iconic_WDS_Helpers::is_same_day( $timestamp );
	 ...
	 * @return array
	 */
	public function create_timestamp_range( $timestamp_from, $timestamp_to ) {
		$range = array();

		if ( $timestamp_to >= $timestamp_from ) {
			if ( $this->is_delivery_day( $timestamp_from ) ) {
	...
	public function get_allowed_delivery_days( $minmax = false ) {
		...
		if ( ! $minmax || 'allowed' === $mixmax_method ) {
			$chosen_days = Iconic_WDS_Settings::get_delivery_days( 'frontend', $this->cart->get_products_ids() );
	...
	public function is_delivery_slots_allowed() {
		if ( Iconic_WDS_Edit_Timeslots::is_sub_order( $this->order ) ) {
			/**
		...
		if ( ! Iconic_WDS_Checkout::is_delivery_slots_allowed_for_shipping_method( $this->shipping_method ) ) {
			/**
			 * Is delivery slots allowed i.e. whether to display the delivery slots fields.
			 *
			 * @since 1.0.0
	...
	public function slots_available_on_date( $ymd, $include_booked_out_slots = false ) {
		...
		foreach ( $timeslots as $timeslot ) {
			...
			$slot_allowed_for_method = $iconic_wds->is_timeslot_allowed_for_method( $timeslot, $this->shipping_method );
	...
	public function display_checkout_fields( $order_id = false ) {
		...
		$bookable_dates = $this->get_upcoming_bookable_dates( Iconic_WDS_Helpers::date_format() );
	...
	public function get_checkout_fields_data( $order = null ) {
		...
		$fields['jckwds-delivery-date'] = array(
			...
			'field_args' => array(
				...
				'description'       => $iconic_wds->settings['datesettings_datesettings_setup_show_description'] ? Iconic_WDS_Helpers::get_label( 'choose_date', $order ) : false,
			),
		);
		...
		if ( $iconic_wds->settings['timesettings_timesettings_setup_enable'] ) {
			$fields['jckwds-delivery-time'] = array(
				...
				'field_args' => array(
					...
					'options'     => array(
						'' => Iconic_WDS_Helpers::get_label( 'select_date_first' ),
					),
					'description' => $iconic_wds->settings['timesettings_timesettings_setup_show_description'] ? Iconic_WDS_Helpers::get_label( 'choose_time_slot', $order ) : false,
				),
			);
			...
			if ( ! empty( $reserved ) && ! empty( $reserved['time'] ) ) {
				...
				$fields['jckwds-delivery-time']['field_args']['options'] = array(
					'' => Iconic_WDS_Helpers::get_label( 'select_time_slot' ),
				);
	...
	public function needs_shipping() {
		if ( ! empty( Iconic_WDS_Checkout::display_for_virtual_products() ) ) {
			return true;
		}
	 ...
	 * @return array
	 */
	public function get_calculated_fees_data( $delivery_date, $delivery_date_ymd, $delivery_time ) {
		global $iconic_wds;
		...
		if ( ! empty( $delivery_time ) ) {
			...
			if ( $timeslot_fee > 0 ) {
				$fees['jckwds_timeslot_fee'] = $timeslot_fee;
			} else {
				$fees['jckwds_timeslot_fee'] = false;
			}
		} else {
			$fees['jckwds_timeslot_fee'] = false;
		}
		...
		if ( $iconic_wds->settings['datesettings_fees_same_day'] > 0 ) {
			$same_day = $this->get_same_day_date( Iconic_WDS_Helpers::date_format() );
			if ( ! empty( $delivery_date ) && $delivery_date === $same_day ) {
				$fees['jckwds_same_day_fee'] = $iconic_wds->settings['datesettings_fees_same_day'];
			} else {
				$fees['jckwds_same_day_fee'] = false;
			}
		} else {
			$fees['jckwds_same_day_fee'] = false;
		}
		...
		if ( $iconic_wds->settings['datesettings_fees_next_day'] > 0 ) {
			$next_day = $this->get_next_day_date( Iconic_WDS_Helpers::date_format() );
			if ( ! empty( $delivery_date ) && $delivery_date === $next_day ) {
				$fees['jckwds_next_day_fee'] = $iconic_wds->settings['datesettings_fees_next_day'];
			} else {
				$fees['jckwds_next_day_fee'] = false;
			}
		} else {
			$fees['jckwds_next_day_fee'] = false;
		}
