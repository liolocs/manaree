...
class Iconic_WDS_Ajax {
	...
	public static function reserve_slot() {
		...
		$date_formatted = date_i18n( Iconic_WDS_Helpers::date_format(), strtotime( $slot_date ) );
		$iconic_wds->update_fees_session( $date_formatted, $slot_date, $date_time );
	...
	public static function get_upcoming_bookable_dates() {
		...
		$format = filter_input( INPUT_POST, 'format' ) ? filter_input( INPUT_POST, 'format' ) : Iconic_WDS_Helpers::date_format();
	...
	public static function get_address_shipping_methods() {
		...
		$data = Iconic_WDS_Helpers::get_allowed_shipping_methods_for_address( $country, $state, $city, $postcode );
		...
		Iconic_WDS_Reservation_Table::calculate_shipping();
	...
	public static function get_reservation_table_data() {
		...
		$dates_manager = new Iconic_WDS_Dates( array( 'shipping_method' => $shipping_method[0] ) );
		...
		$bookable_dates = Iconic_WDS_Reservation_Table::add_fees_to_dates( $bookable_dates );
		$bookable_dates = Iconic_WDS_Helpers::remove_duplicates_in_assoc_by_key( $bookable_dates, 'ymd' );
		...
		*/
		...
		$timeslot_output = Iconic_WDS_Reservation_Table::get_timeslot_data( $batch_dates, $dates_manager );

		wp_send_json_success(
			array(
				'dates'                => $bookable_dates,
				'timeslots'            => $timeslot_output,
				'shipping_method_type' => 'delivery' === Iconic_WDS_Helpers::get_shipping_method_type( $shipping_method[0] ) ? esc_html_x( 'Delivery', 'Reservation calendar', 'jckwds' ) : esc_html_x( 'Collection', 'Reservation calendar', 'jckwds' ),
	...
	public static function update_order_slot() {
		...
		if ( ! Iconic_WDS_Edit_Timeslots::can_current_user_edit_timeslot( $order, $context ) ) {
			wp_send_json_error(
				array(
		...
		$data = array(
			'jckwds-delivery-date'     => date_i18n( Iconic_WDS_Helpers::date_format(), strtotime( filter_input( INPUT_POST, 'date_ymd', FILTER_SANITIZE_FULL_SPECIAL_CHARS ) ) ),
		...
		Iconic_WDS_Order::update_order_meta( $order_id, $data );
		...
		// Add/Update fees.
		$order = Iconic_WDS_Admin::save_fee_to_order( $fees, $order );
	...
	public static function admin_delete_order_slot() {
		...
		Iconic_WDS_Order::cancel_order( $order_id );

		// Delete fee.
		$order = Iconic_WDS_Admin::save_fee_to_order( 0, $order );
	...
	public static function check_fee_difference() {
		...
		$difference_arr     = Iconic_WDS_Edit_Timeslots::get_fee_difference( $order, $date, $ymd, $time );
		...
		if ( ! $charge_fee_enabled || $difference <= $threshold || $difference < 0 ) {
			Iconic_WDS_Order::update_order_meta(
				$order->get_id(),
				array(
					'jckwds-date-changed' => 1,
				)
	...
	public static function create_sub_order() {
		...
		if ( ! empty( $session_data ) && ! empty( $session_data['sub_order_id'] ) ) {
			...
			if ( ! empty( $sub_order ) && 'pending' === $sub_order->get_status() ) {
				Iconic_WDS_Edit_Timeslots::prepare_sub_order( $parent_order, $sub_order, $date, $ymd, $time );
		...
		$sub_order = Iconic_WDS_Edit_Timeslots::prepare_sub_order( $parent_order, null, $date, $ymd, $time );
