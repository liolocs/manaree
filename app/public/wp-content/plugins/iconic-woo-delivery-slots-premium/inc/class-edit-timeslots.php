 ...
 * WDS Ajax class.
 */
class Iconic_WDS_Edit_Timeslots {
	...
	public static function maybe_display_checkout_fields( $order = false ) {
		...
		if (
			Iconic_WDS_Helpers::is_thankyou_page()
			&&
		...
		if ( Iconic_WDS_Helpers::is_my_account_order_page() && 'allow' !== $iconic_wds->settings['general_customer_display_on_myaccount_order_page'] ) {
			return;
		}
		...
		$date_manager = new Iconic_WDS_Dates( array( 'order_id' => $order->get_id() ) );
		...
		$bookable_dates    = $date_manager->get_upcoming_bookable_dates( Iconic_WDS_Helpers::date_format() );
		$show_save_button  = Iconic_WDS_Helpers::is_thankyou_page() || Iconic_WDS_Helpers::is_my_account_order_page();
		$order_id          = $order->get_id();
		$has_timeslot_data = Iconic_WDS_Order::get_order_date_time( $order );
	...
	public static function get_update_window_in_seconds() {
		global $iconic_wds;
		$window = Iconic_WDS_Settings::get_timeslot_editing_window();
	...
	public static function get_wds_fee_amount( $order ) {
		...
		$fees             = $order->get_fees();
		$search_fee_label = $iconic_wds->get_fee_name();
		foreach ( $fees as $fee ) {
	...
	public static function get_fee_difference( $order, $date, $ymd, $time ) {
		...
		$new_fee     = $date_manager->get_calculated_fees_data( $date, $ymd, $timeslot['value'] );
	...
	public static function after_suborder_pay( $sub_order_id ) {
		...
		Iconic_WDS_Order::update_order_meta( $parent_order, $data );
	}
	...
	public static function get_child_orders( $order ) {
		...
		if ( Iconic_WDS_Helpers::is_cot_enabled() ) {
