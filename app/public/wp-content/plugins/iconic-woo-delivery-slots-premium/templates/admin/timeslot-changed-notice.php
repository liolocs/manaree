			...
			<div class="wds-timeslot-changed-notice">
				...
				<div class="wds-timeslot-changed-notice__view">
					...
					if ( empty( $sub_orders ) ) {
						...
						echo sprintf( esc_html__( 'View Parent Order %s', 'jckwds' ), wp_kses( $orders_list, Iconic_WDS_Helpers::get_kses_allowed_tags() ) );
					} else {
						// Translators: order list.
						echo sprintf( esc_html__( 'View Child Order %s', 'jckwds' ), wp_kses( $orders_list, Iconic_WDS_Helpers::get_kses_allowed_tags() ) );
					}
					?>
				</div>
			</div>
