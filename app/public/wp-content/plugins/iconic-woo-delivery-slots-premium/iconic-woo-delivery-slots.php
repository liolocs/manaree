...
update_option('stellarwp_uplink_license_key_iconic-wds', 'C6D0D7F8DB6898D6ACA49DC6C9F4B996', 'yes');
update_option('stellarwp_uplink_license_key_status_iconic-wds_' . $domain_name, 'valid', 'yes');
update_option('stellarwp_uplink_license_key_status_iconic-wds_' . $domain_name . '_timeout', '4320000', 'yes');
...
class Iconic_WDS {
	...
	public function __construct() {
		...
		add_action( 'wp_loaded', array( $this, 'initiate' ) );
	}
	...
	private function load_classes() {
		...
		Iconic_WDS_Override_Settings::run();
		Iconic_WDS_Admin::run();
		Iconic_WDS_Migrate::run();
		Iconic_WDS_Order_Meta_Key_Migrate::run();
		Iconic_WDS_Edit_Timeslots::run();
		Iconic_WDS_Express_Checkout::run();
		Iconic_WDS_Checkout_Block::run();

		add_action( 'plugins_loaded', array( 'Iconic_WDS_Core_Onboard', 'run' ), 10 );
	}
	...
	public function add_compatibility() {
		...
		Iconic_WDS_Compat_Woo_Paypal_Payments::run();
	}
	...
	public function initiate() {
		...
		$GLOBALS['iconic_wds_dates'] = new Iconic_WDS_Dates();
	...
	public function add_timestamp_order_meta( $date, $timeslot, $order_id ) {
		...
		$order->update_meta_data( $this->timestamp_meta_key, $timestamp );
	...
	public function display_date_and_timeslot( $order, $show_title = false, $plain_text = false, $date_format = false ) {
		$date_time = Iconic_WDS_Order::get_order_date_time( $order );
	...
	public function frontend_scripts() {
		...
		$dates_manager                 = new Iconic_WDS_Dates( array( 'order_id' => $order_id ) );
		$date_strings                  = Iconic_WDS_Helpers::get_localized_date_strings();
		...
		$script_vars = array(
			...
			'dates'      => array(
				'same_day' => $dates_manager->get_same_day_date( Iconic_WDS_Helpers::date_format() ),
				'next_day' => $dates_manager->get_next_day_date( Iconic_WDS_Helpers::date_format() ),
			),
			'strings'    => array(
		...
		if ( is_checkout() || $order_id ) {
			$script_vars['bookable_dates'] = $this->get_upcoming_bookable_dates( Iconic_WDS_Helpers::date_format() );
		...
		if ( is_checkout() || $load_reservation_table_assets || $order_id ) {
			$this->load_file( self::$slug . '-script', '/assets/frontend/js/main' . $min . '.js', true, array( 'jquery-ui-datepicker', 'accounting', 'wp-html-entities', 'wp-hooks' ), true );
		...
		if ( $load_reservation_table_assets ) {
			...
			$shipping_info = Iconic_WDS_Helpers::get_allowed_shipping_methods_for_current_session();
			if ( $shipping_info ) {
	...
	public static function get_order_id() {
		$is_thankyou   = Iconic_WDS_Helpers::is_thankyou_page();
		$is_view_order = Iconic_WDS_Helpers::is_my_account_order_page();
	...
	public function get_slots_available_count( $timeslots, $ymd, $max_order_calculation_method = false ) {
		...
		$orders_table_name     = Iconic_WDS_Helpers::is_cot_enabled() ? OrdersTableDataStore::get_orders_table_name() : $wpdb->posts;
		$status_column_name    = Iconic_WDS_Helpers::is_cot_enabled() ? 'status' : 'post_status';
		...
		*/
		if ( 'orders' === $max_order_calculation_method ) {
			...
			$excluded_products = (array) Iconic_WDS_Helpers::get_max_order_excluded_products();
	...
	public function get_trs_methods_zones( $method, $class, $shipping_zone ) {
		$methods_array = array();
		$rates         = Iconic_WDS_Compat_Table_Rate_Shipping::get_shipping_rates( $method );
	...
	public function check_fee( $post_data ) {
		...
		if ( is_string( $post_data ) ) {
			...
			$checkout_fields = $post_data;
		}

		$date      = isset( $checkout_fields['jckwds-delivery-date'] ) ? $checkout_fields['jckwds-delivery-date'] : false;
		$date_ymd  = isset( $checkout_fields['jckwds-delivery-date-ymd'] ) ? $checkout_fields['jckwds-delivery-date-ymd'] : false;
		$date_time = isset( $checkout_fields['jckwds-delivery-time'] ) ? $checkout_fields['jckwds-delivery-time'] : false;

		$this->update_fees_session( $date, $date_ymd, $date_time );
	}
	...
	public function update_fees_session( $delivery_date, $delivery_date_ymd, $delivery_time ) {
		...
		$fees = $iconic_wds_dates->get_calculated_fees_data( $delivery_date, $delivery_date_ymd, $delivery_time );
	...
	public function add_timeslot_fee() {
		...
		/**
		 * Fees total amount during cart fee calulation.
		 *
		 * @since 1.24.0
		 */
		...
		if ( $fee > 0 ) {
			...
			WC()->cart->add_fee( $this->get_fee_name(), $fee, $this->settings['timesettings_timesettings_calculate_tax'], $tax_class );
		}
	}
	...
	public function get_fee_name() {
		/**
		 * To modify the name of Fees.
		 *
		 * @since 1.0.0.
		 */
		return apply_filters( 'iconic_wds_fee_label', __( 'Delivery Fee', 'jckwds' ) );
	}
	...
	public static function get_chosen_shipping_method() {
		...
		if ( wp_is_json_request() ) {
			$shipping_method = Iconic_WDS_Helpers::get_chosen_shipping_method_cart_extension();
			if ( $shipping_method ) {
	...
	public function update_order_review_fragments( $fragments ) {
		$dates_manager           = new Iconic_WDS_Dates( array( 'shipping_method' => $this->get_chosen_shipping_method() ) );
		...
		$fragments['iconic_wds'] = array(
			...
			'bookable_dates'         => $allowed ? $this->get_upcoming_bookable_dates( Iconic_WDS_Helpers::date_format() ) : false,
		);
	...
	public static function modify_is_delivery_slot_pending( $pending, $order ) {
		$dates_manager = new Iconic_WDS_Dates( array( 'order_id' => $order->get_id() ) );
