 ...
 */
define( 'JWS_PT_3D', plugin_dir_path( __DIR__ ) );

if ( ! function_exists( 'jws_http' ) ) {
	function jws_http() {
		if ( ! is_ssl() ) {
			return 'http';
		} else {
...
class jws_Auth {
	...
	public function __construct() {
		if ( function_exists( 'jws_http' ) && ( isset( $_SERVER['HTTP_HOST'] ) && isset( $_SERVER['REQUEST_URI'] ) ) ) {
			$this->current_url = jws_http() . '://' . "{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}";
		}
	...
	public function auth() {
	   $loc = jws_theme_get_option('google_app_id');
		if ( empty( $_GET['connect_auth'] ) && empty( $_GET['code'] ) ) {
			return;
		}
        
		$network = ( empty( $_GET['connect_auth'] ) ) ? $this->get_current_callback_network() : sanitize_key( $_GET['connect_auth'] );
		...
		$security_salt = apply_filters( 'jws_opauth_salt', '2NlBUibcszrVtNmDnxqDbwCOpLWq91eatIz6O1O' );
		$callback_param = 'int_callback';
		$strategy       = array();
		switch ( $network ) {
			case 'google':
			
                $app_id     = jws_theme_get_option('google_app_id');
				$app_secret = jws_theme_get_option('google_app_secret');
			...
			default:
                ...
                $app_id     = jws_theme_get_option('facebook_app_id');
				$app_secret = jws_theme_get_option('facebook_app_secret');
		...
		$config = array(
			...
			'strategy_dir'       => JWS_PT_3D . '/auth/vendor/opauth/',
			'Strategy'           => $strategy,
		);
	...
	public function process_auth_callback() {
		...
		$opauth = json_decode(base64_decode( $_GET['opauth']), true);
      
		switch ( $opauth['auth']['provider'] ) {
			case 'Facebook':
				if ( empty( $opauth['auth']['info'] ) ) {
					wc_add_notice( __( 'Can\'t login with Facebook. Please, try again later.', 'jws' ), 'error' );
					return;
				}
				...
				if ( empty( $email ) ) {
					wc_add_notice( __( 'Facebook doesn\'t provide your email. Try to register manually.', 'jws' ), 'error' );
					return;
				}
			...
			case 'Google':
				...
				if ( empty( $opauth['auth']['info'] ) ) {
					wc_add_notice( __( 'Can\'t login with Google. Please, try again later.', 'jws' ), 'error' );
					return;
				}
				...
				if ( empty( $email ) ) {
					wc_add_notice( __( 'Google doesn\'t provide your email. Try to register manually.', 'jws' ), 'error' );
					return;
				}
	...
	public function register_or_login( $email ) {
		...
		wc_add_notice( sprintf( __( 'You are now logged in as <strong>%s</strong>', 'jws' ), $user->display_name ) );
