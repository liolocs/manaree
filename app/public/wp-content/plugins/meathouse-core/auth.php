...
class jws_Auth {
	...
	public function auth() {
		...
		switch ( $network ) {
			case 'google':
				$app_id     =  jws_theme_get_option( 'goo_app_id' );
				$app_secret = jws_theme_get_option( 'goo_app_secret' );
			...
			default:
				$app_id     = jws_theme_get_option( 'fb_app_id' );
				$app_secret = jws_theme_get_option( 'fb_app_secret' );
		...
		$config = array(
			...
			'strategy_dir'       => JWS_PT_3D . '/vendor/opauth/',
			'Strategy'           => $strategy,
		);
		...
		switch ( $opauth['auth']['provider'] ) {
			case 'Facebook':
				if ( empty( $opauth['auth']['info'] ) ) {
					wc_add_notice( __( 'Can\'t login with Facebook. Please, try again later.', 'auriane' ), 'error' );
					return;
				}
				...
				if ( empty( $email ) ) {
					wc_add_notice( __( 'Facebook doesn\'t provide your email. Try to register manually.', 'auriane' ), 'error' );
					return;
				}
			...
			case 'Google':
				if ( empty( $opauth['auth']['info'] ) ) {
					wc_add_notice( __( 'Can\'t login with Google. Please, try again later.', 'auriane' ), 'error' );
					return;
				}
				...
				if ( empty( $email ) ) {
					wc_add_notice( __( 'Google doesn\'t provide your email. Try to register manually.', 'auriane' ), 'error' );
					return;
				}
			...
			case 'VKontakte':
				if ( empty( $opauth['auth']['info'] ) ) {
					wc_add_notice( __( 'Can\'t login with VKontakte. Please, try again later.', 'auriane' ), 'error' );
					return;
				}
				...
				if ( empty( $email ) ) {
					wc_add_notice( __( 'VK doesn\'t provide your email. Try to register manually.', 'auriane' ), 'error' );
					return;
				}
	...
	public function register_or_login( $email ) {
		...
		wc_add_notice( sprintf( __( 'You are now logged in as <strong>%s</strong>', 'auriane' ), $user->display_name ) );
