	...
	public function swatch_html( $html, $term, $type, $args, $product ) {
		...
		switch ( $type ) {
			...
			case 'label':
				...
				$label = $label ?: $name;
				$html = sprintf(
					'<div class="swatch-item-wrapper"><div class="swatch swatch-shape-' . $swatchShape . ' swatch-label swatch-%s %s" data-value="%s"><span class="text-label">%s</span><span class="text">%s</span></div>%s</div>',
					esc_attr( $term->slug ),
					$selected,
					esc_attr( $term->slug ),
					esc_html( $label ),
