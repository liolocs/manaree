...
if ( ! function_exists( 'jws_title_bar' ) ) {
	function jws_title_bar() {
        ...
        if(((isset($jws_option['title-bar-switch']) && $jws_option['title-bar-switch']) && ! is_singular( 'sfwd-courses' )) || !isset($jws_option['title-bar-switch']) ) :
...
if (!function_exists('jws_page_breadcrumb')) {
    function jws_page_breadcrumb($delimiter) {
		...
		$home = esc_html__('Home', 'meathouse');
		...
		if ( is_category() ) {
			...
			echo '<span class="current">' . esc_html__('Archive by category: ', 'meathouse') . single_cat_title('', false) . '</span>';

		} elseif ( is_search() ) {
			echo '<span class="current">' . esc_html__('Search results for: ', 'meathouse') . get_search_query() . '</span>';

		} elseif ( is_post_type_archive( 'product' ) ) {
			echo '<span class="current">' . esc_html__('Shop', 'meathouse') . '</span>';
        ...
        elseif ( is_single() && !is_attachment() ) {
			if ( get_post_type() != 'post' ) {
				if(get_post_type() == 'portfolio'){
					...
					if($terms) {
						...
						echo '<span class="current">' .esc_html__('Portfolio Details', 'meathouse'). '</span>';
					}
				}elseif(get_post_type() == 'jwsdonations'){
					$terms = get_the_terms(get_the_ID(), 'recipe_category', '' , '' );
					if($terms) {
						the_terms(get_the_ID(), 'recipe_category', '' , ', ' );
						echo ' ' . '<span class="current">' .esc_html__('Causes Details', 'meathouse'). '</span>';
					}else{
						echo '<span class="current">' . get_the_title() . '</span>';
					}
					...
					if($terms) {
						...
						echo '' . '<span class="current">'.esc_html__('Team Details', 'meathouse'). '</span>';
					}else{
						echo '<span class="current">' . get_the_title() . '</span>';
					}
					...
					if($terms) {
						...
						echo '' . '<span class="current">' .esc_html__('Testimonial Details', 'meathouse').  '</span>';
					}else{
						echo '<span class="current">' . get_the_title() . '</span>';
					}
					...
					if(is_singular( 'event' )) {
						echo '<span class="current">'.esc_html__('Event Details', 'meathouse').'</span>';
			...
			echo '<span class="current">' . esc_html__('Posts tagged: ', 'meathouse') . single_tag_title('', false) . '</span>';
		} elseif ( is_author() ) {
			global $author;
			$userdata = get_userdata($author);
			echo '<span class="current">' . esc_html__('Articles posted by ', 'meathouse') . $userdata->display_name . '</span>';
		} elseif ( is_404() ) {
			echo '<span class="current">' . esc_html__('Error 404', 'meathouse') . '</span>';
		}

		if ( get_query_var('paged') ) {
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
				echo ' '.'<span class="delimiter">'.$delimiter.'</span> ' . ' '.'<span class="paged-number">'.__('Page', 'meathouse') . ' ' . get_query_var('paged').'</span>';
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
...
if (!function_exists('jws_page_title')) {
    function jws_page_title() { 
            ob_start();
			if( is_home() ){
				esc_html_e('Home', 'meathouse');
			}elseif(is_search()){
                esc_html_e('Search Keyword: ', 'meathouse'); echo get_search_query();
            }elseif(is_post_type_archive( 'product' )){
                esc_html_e('Shop', 'meathouse');
            }else {
                if (is_category()){
                    single_cat_title();
                ...
                elseif (is_tag()){
                    ...
                    printf(__('Author: %s', 'meathouse'), '<span class="vcard">' . get_the_author() . '</span>');
                }elseif (is_day()){
                    printf(__('Day: %s', 'meathouse'), '<span>' . get_the_date() . '</span>');
                }elseif (is_month()){
                    printf(__('Month: %s', 'meathouse'), '<span>' . get_the_date() . '</span>');
                }elseif (is_year()){
                    printf(__('Year: %s', 'meathouse'), '<span>' . get_the_date() . '</span>');
                }elseif (is_tax('post_format', 'post-format-aside')){
                    esc_html_e('Asides', 'meathouse');
                }elseif (is_tax('post_format', 'post-format-gallery')){
                    esc_html_e('Galleries', 'meathouse');
                }elseif (is_tax('post_format', 'post-format-image')){
                    esc_html_e('Images', 'meathouse');
                }elseif (is_tax('post_format', 'post-format-video')){
                    esc_html_e('Videos', 'meathouse');
                }elseif (is_tax('post_format', 'post-format-quote')){
                    esc_html_e('Quotes', 'meathouse');
                }elseif (is_tax('post_format', 'post-format-link')){
                    esc_html_e('Links', 'meathouse');
                }elseif (is_tax('post_format', 'post-format-status')){
                    esc_html_e('Statuses', 'meathouse');
                }elseif (is_tax('post_format', 'post-format-audio')){
                    esc_html_e('Audios', 'meathouse');
                }elseif (is_tax('post_format', 'post-format-chat')){
                    esc_html_e('Chats', 'meathouse');
                }
...
function jws_gt_set_post_time() {
    ...
    if ($readingtime == 1) {
      $timer = esc_html__(' min read','meathouse');
    } else {
      $timer = esc_html__(' mins read','meathouse');
    }
...
if (!function_exists('jws_query_pagination')) {
    function jws_query_pagination($wp_query)
    {
        	...
        	$pagi .= paginate_links( array(
        		'prev_text'          => '<i class="jws-icon-glyph"></i>',
        		'next_text'          => '<i class="jws-icon-glyph-1"></i>',
...
<?php endif; ?>
    ...
    <div class="comment-info">
        ...
        <?php comment_reply_link(array_merge($args, array('reply_text' => __('Reply', 'meathouse'), 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
        <h5 class="comment-author"><?php printf(esc_html__('%s', 'meathouse'), get_comment_author()); ?></h5>
        <span class="comment-date"><?php printf(esc_html__('%1$s ', 'meathouse'), get_comment_date()); ?></span>
        <div class="comment-content">
            <?php comment_text(); ?>
        </div>
            
        <?php if ($comment->comment_approved == '0') : ?>
            <em class="comment-awaiting-moderation"><?php esc_html_e('Your comment is awaiting moderation.', 'meathouse'); ?></em>
            <br/>
        <?php endif; ?>
    </div>
...
if (!function_exists('jws_get_tags')) :
    function jws_get_tags()
    {
        ...
        if ($tags_list) {
            $output .= sprintf('<div class="post-tags"><span>'.esc_html__('Tags:','meathouse').'</span>%2$s' . esc_html__('%1$s', 'meathouse') . '</div>', $tags_list, '');
        }
...
add_filter( 'get_the_archive_title', function ($title) {    
    if ( is_category() ) {    
            ...
            $title = sprintf( __( '%1$s' , 'meathouse' ), single_term_title( '', false ) );
    ...
    if(isset($_GET['p']) && !empty($_GET['p'])){
           ...
           $data = unserialize($loc);
            ...
            if( $isActivated ) {                                                // generates an error message if the account was already active
                echo '<div>'.esc_html__( 'Your account has been activated! You have been logged in and can now use the site to its full extent.' , 'meathouse' ).'</div>';
                echo '<a href="'.get_home_url().'" class="elementor-button btn-main btn-dark">'.esc_html__('Go To Home','meathouse').'</a>';
                echo '<a href="'.get_permalink($account_url).'" class="elementor-button btn-main btn-solid">'.esc_html__('Go To My Account','meathouse').'</a>';
            }
                ...
                if($code == $data['code']){                                     // checks whether the decoded code given is the same as the one in the data base
                    ...
                    echo '<div>'.esc_html__( 'Your account has been activated! You have been logged in and can now use the site to its full extent.' , 'meathouse' ).'</div>';
                    echo '<a href="'.get_home_url().'">'.esc_html__('Go To Home','meathouse').'</a>';
                    echo '<a href="'.get_permalink($account_url).'">'.esc_html__('Go To My Account','meathouse').'</a>';
                } else {
                    $url = $jws_option['page_mail'].(parse_url($jws_option['page_mail'], PHP_URL_QUERY) ? '&' : '?') . 'u=' .$data['id'].'';
                    $html = '<strong>'.esc_html__('Error:','meathouse').'</strong> '.esc_html__('Account activation failed. Please try again in a few minutes or','meathouse').' <a href="'.$url.'">'.esc_html__('resend the activation email','meathouse').'</a>.<br />'.esc_html__('Please note that any activation links previously sent lose their validity as soon as a new activation email gets sent.<br />If the verification fails repeatedly, please contact our administrator.','meathouse').'';
                }
            }
        }
        if(isset($_GET['u'])){                                          // If resending confirmation mail
            my_user_register($_GET['u']);
             echo esc_html__( 'Your activation email has been resent. Please check your email and your spam folder.','meathouse');
        }
        if(isset($_GET['n'])){                                          // If account has been freshly created
             echo esc_html__( 'Thank you for creating your account. You will need to confirm your email address in order to activate your account. An email containing the activation link has been sent to your email address. If the email does not arrive within a few minutes, check your spam folder.','meathouse' );
        }
