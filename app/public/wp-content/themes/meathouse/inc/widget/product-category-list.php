...
class JWS_PRODUCT_CATEGORY_class extends WC_Widget {
	...
	function __construct() {
        ...
        $this->widget_cssclass    = 'widget-meathouseduct-category';
		$this->widget_description = __( 'It Displays Product Category List', 'meathouse' );
		$this->widget_id          = 'widget-meathouseduct-category';
		$this->widget_name        = __( 'JWS Product Category List', 'meathouse' );
	...
	public function init_settings() {
	   ...
	   $this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Product categories', 'meathouse' ),
				'label' => __( 'Title', 'meathouse' ),
			),
            'hide_empty' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Hide empty categories', 'meathouse' ),
			),
			'show_parent_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Only show parent category', 'meathouse' ),
			),
            'hidden_category' => array(
				'type'  => 'select',
				'std'     => 'name',
				'label'   => __( 'Hidden category default','meathouse' ),
				'options'   => $product_cat
			),
		);
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="meathouseduct-category-list">
           <div class="container">
               <?php 
                    if(!empty($title)) {
