...
class Jws_Widget_Attributes_Filter extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'meathouse' );
		$this->widget_id          = 'jws_attributes_filter';
		$this->widget_name        = esc_html__( 'Jws Attributes Filter', 'meathouse' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title'      => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by', 'meathouse' ),
				'label' => esc_html__( 'Title', 'meathouse' )
			),
			'attribute'  => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => esc_html__( 'Attribute', 'meathouse' ),
				'options' => $attribute_array
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => esc_html__( 'Query type', 'meathouse' ),
				'options' => array(
					'and' => esc_html__( 'AND', 'meathouse' ),
					'or'  => esc_html__( 'OR', 'meathouse' )
				)
			),
		);
	}
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type, $show_color ) {
		...
		foreach ( $terms as $term ) {
			...
			echo ''.$count > 0 ? apply_filters( 'jws_color_filter_nav_count', '<span class="count-atr">' . absint( $count ) . '</span>', $count, $term ) : '';
