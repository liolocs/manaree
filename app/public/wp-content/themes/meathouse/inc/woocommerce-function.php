<?php
        ...
        add_filter( 'meathouse_after_single_product_image', 'product_thumbnails' );
        // Add Shop Toolbar
	...
	function instance_search_result() {
		...
		if ( empty( $response ) ) {
			$response[] = sprintf( '<li>%s</li>', esc_html__( 'Nothing found', 'meathouse' ) );
		}
	...
	function product_thumbnails() {
		global $post, $product, $woocommerce , $meathouse_loop;
        $style = '1' ;
        $thumb_position =  'left' ;
        ...
        if($style == 1 || $style == 6 || $style == 8 || (isset($meathouse_loop['loc']) && $meathouse_loop['loc'] == 'loc') ) {
       	if ( $attachment_ids ) {
			$loop    = 1;
	...
	function shop_toolbar() {
		...
		$filters = '';
			$filters = sprintf( '<a href="#" class="jws-filter filters"><span>+</span> %s</a>', esc_html__( 'Filters', 'meathouse' ) );
		$found = '';
		$shop_view = '';
        ...
        $found = '';
			...
			if ( $wp_query && isset( $wp_query->found_posts ) ) {
				$found = '<span>' . $wp_query->found_posts . ' </span>' . esc_html__( 'Products Found', 'meathouse' );
			}
			if ( $found ) {
...
if ( ! function_exists( 'jws_ajax_suggestions' ) ) {
            	function jws_ajax_suggestions() {
            		...
            		if ( $post_type == 'product') {
                        ...
                        $option_exclude_cat = jws_theme_get_option('exclude-category-in-shop');
                       if(!empty($option_exclude_cat)) {
                            $query_args['tax_query'] = array( array(
                                'taxonomy' => 'product_cat',
                                'field'    => 'term_id',
                                'terms'    => implode(',', $option_exclude_cat), // Term ids to be excluded
            		...
            		if ( $results->have_posts() ) {
            			...
            			$suggestions[] = array(
            				'value' => ( $post_type == 'product' ) ? esc_html__( 'No products found', 'meathouse' ) : esc_html__( 'No posts found', 'meathouse' ),
            				'no_found' => true,
            				'permalink' => ''
            			);
            		}
