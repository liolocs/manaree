                ...
                <div class="section-form-left">
                    <div class="form-contaier">
            			...
            			<div class="jws-login slick-slide">
                            <h3 class="heading-form"><?php echo esc_html__('Login','meathouse') ?></h3>
            				<form name="loginpopopform" id="loginform" action="<?php echo esc_url( site_url( 'wp-login.php', 'login_post' ) ); ?>" method="post">
                            
            					<p class="login-username">
                                    <label><?php esc_attr_e( 'Username or email *', 'meathouse' ); ?></label>
            						<input type="text" name="log" class="input required" value="" size="20" />
            					</p>
            					<p class="login-password">
                                    <label><?php esc_attr_e( 'Password *', 'meathouse' ); ?></label>
            						<input type="password" name="pwd" class="input required" value="" size="20" />
                                    <span class="field-icon toggle-password2 jws-icon-eye-thin"></span>
            					</p>
            				
                                <p class="forgetmenot login-remember">
            						<label for="popupRememberme"><input name="rememberme" type="checkbox" value="forever" id="popupRememberme" /> <?php esc_html_e( 'Remember me', 'meathouse' ); ?>
                                    
            						</label><?php echo '<a class="lost-pass-link" href="' . $wc_lostpassword_url . '" title="' . esc_attr__( 'Lost Password', 'meathouse' ) . '">' . esc_html__( 'Lost your password?', 'meathouse' ) . '</a>'; ?>
            					</p>
            					<div class="g-recaptcha" id="recaptcha7"></div>
            					<p class="submit login-submit jws-button">
            						<input type="submit" name="wp-submit" class="button elementor-button" value="<?php esc_attr_e( 'Sign In', 'meathouse' ); ?>" />
            						<input type="hidden" name="testcookie" value="1" />
            					</p>
                            <?php
            				if ( $show_login && ($show_register && $registration_enabled) ) {
            					echo '<a class="register change-form" href="' . esc_url( jws_get_register_url() ) . '">' . esc_html__( 'Sign up for a new account here.', 'meathouse' ) .'</a>';
            				}
            				?>
            			...
            			<?php if ( $registration_enabled && $show_register ): ?>
            				<div class="jws-register swiper-slide">
                                <h3 class="heading-form"><?php echo esc_html__('Register','meathouse') ?></h3>
            					<form class="auto_login" name="registerformpopup"  method="post" novalidate="novalidate">
            						...
            						<p class="col-xl-6 col-lg-6 col-12">
                                        <label><?php esc_attr_e( 'Username *', 'meathouse' ); ?></label>
            							<input  type="text" name="user_login" class="input required" />
            						</p>
            
            						<p class="col-xl-6 col-lg-6 col-12">
                                        <label><?php esc_attr_e( 'Email *', 'meathouse' ); ?></label>
            							...
            							<p>
                                            <label><?php esc_attr_e( 'Repeat Password *', 'meathouse' ); ?></label>
            						...
            						<p class="submit jws-button">
            							<input type="submit" name="wp-submit" class="button elementor-button" value="<?php echo esc_attr_x( 'Sign up', 'Login popup form', 'meathouse' ); ?>" />
            						</p>
            					</form>
            				    <?php  if($show_login && ($show_register && $registration_enabled)) echo '<a class="login change-form" href="' . esc_url( jws_get_login_page_url() ) . '">' . esc_html__( 'If you already have an account, log in here.', 'meathouse' ) .'</a>'; ?>
                                  <div class="meter">
                                    <div class="meter-box">
                                  ...
                                  <div class="jws-password-hint">
                                       <?php echo esc_html__('Hint: The password should be at least eight characters long. To make it stronger, use upper and lower case letters, numbers, and symbols like ! " ? $ % ^ & ).','meathouse'); ?>
                                 </div>
            					<div class="popup-message"></div>
            				</div>
            			<?php endif; ?>
                ...
                <div class="section-form-right">
                     ...
                     <?php if ( $show_login && ($show_register && $registration_enabled) ) : ?>      
                         <div class="section-register">
                            <h4><?php echo esc_html__('Create an Account','meathouse'); ?></h4>
                            <p><?php echo esc_html__('Sign up for a new account','meathouse'); ?></p>
                            <a class="register change-form" href="<?php echo esc_url( jws_get_register_url() ); ?>"><?php echo esc_html__( 'Register', 'meathouse' ); ?></a>
                         </div>
                     <?php endif; ?> 
                     <?php if($show_login && ($show_register && $registration_enabled)) : ?>   
                         <div class="section-login">
                            <h4><?php echo esc_html__('Login','meathouse'); ?></h4>
                            <p><?php echo esc_html__('If you already have an account, log in here.','meathouse'); ?></p>
                            <a class="login change-form" href="<?php echo esc_url( jws_get_login_page_url() ); ?>"><?php echo esc_html__( 'Login', 'meathouse' ); ?></a>
                         </div> 
                     <?php endif; ?>  
                     <div class="login-width-social">
                        <div class="social-line">
                            <span><?php echo esc_html__('or','meathouse'); ?></span>
                        </div>
                        <div class="social-list">
                            <a class="jws-connect-google" href="<?php echo add_query_arg( 'connect_auth', 'google', $account_link ); ?>"><?php echo esc_html__('Continue with Google','meathouse'); ?></a>
                            <a class="jws-connect-facebook" href="<?php echo add_query_arg( 'connect_auth', 'facebook', $account_link ); ?>"><?php echo esc_html__('Continue with Facebook','meathouse'); ?></a>
                        </div>  
                    </div>
                </div>
            </div>
	...
	function jws_register_ajax_callback() {
		...
		$info['user_pass']  = sanitize_text_field( $data['password'] );
          ...
          if(!empty($info['user_login']) && isset( $info['user_login'] )) { 
                if (mb_strlen($info['user_login']) < 3) {
                        $response_data = array(
    					   'message' => esc_html__( 'Your User Name Must Contain At Least 3 Characters!', 'meathouse' )
    				    );
                        wp_send_json_error( $response_data );
                }
        }
        ...
        if(!empty($info['user_pass']) && isset( $info['user_pass'] )) {
                ...
                if (mb_strlen($info['user_pass']) < 8) {
                    $response_data = array(
					   'message' => esc_html__( 'Your Password Must Contain At Least 8 Characters!', 'meathouse' )
				    );
                    wp_send_json_error( $response_data );
                }
                ...
                elseif(!preg_match("#[A-Z]+#",$password)) {
                    $response_data = array(
					   'message' => esc_html__( 'Your Password Must Contain At Least 1 Capital Letter!', 'meathouse' )
				    );
                    wp_send_json_error( $response_data );
                }
                ...
                elseif (strcmp($password, $cpassword) !== 0) {
                    $response_data = array(
					   'message' => esc_html__( 'Passwords must match!', 'meathouse' )
				    );
                    wp_send_json_error( $response_data );
                }
		...
		if ( is_wp_error( $user_register ) ) {
			...
			if ( in_array( 'empty_username', $error ) ) {
				$response_data = array(
					'message' => '<p class="jws-dealer-note red">' . esc_html__( 'Please enter a username!', 'meathouse' ) . '</p>'
				);
			}elseif ( in_array( 'empty_password', $error ) ) {
				$response_data = array(
					'message' => '<p class="jws-dealer-note red">' . esc_html__( 'Please enter a password!', 'meathouse' ) . '</p>'
				);
			} elseif ( in_array( 'invalid_username', $error ) ) {
				$response_data = array(
					'message' => '<p class="jws-dealer-note red">' . esc_html__( 'The username is invalid. Please try again!', 'meathouse' ) . '</p>'
				);
			} elseif ( in_array( 'username_exists', $error ) ) {
				$response_data = array(
					'message' => '<p class="jws-dealer-note red">' . esc_html__( 'This username is already registered. Please choose another one.', 'meathouse' ) . '</p>'
				);
			} elseif ( in_array( 'empty_email', $error ) ) {
				$response_data = array(
					'message' => '<p class="jws-dealer-note red">' . esc_html__( 'Please type your e-mail address!', 'meathouse' ) . '</p>'
				);
			} elseif ( in_array( 'invalid_email', $error ) ) {
				$response_data = array(
					'message' => '<p class="jws-dealer-note red">' . esc_html__( 'The email address isn\'t correct. Please try again!', 'meathouse' ) . '</p>'
				);
			} elseif ( in_array( 'email_exists', $error ) ) {
				$response_data = array(
					'message' => '<p class="jws-dealer-note red">' . esc_html__( 'This email is already registered. Please choose another one!', 'meathouse' ) . '</p>'
				);
			}
			...
			wp_send_json_error( $response_data );
                ...
                if($jws_option['verify_email'] && !empty($jws_option['page_mail'])) {
                    ...
                    $response_data = array(
    				    'code'    => $code,
        				'message' => '<p class="jws-dealer-note green">' . esc_html__( 'Login successful, redirecting...', 'meathouse' ) . '</p>',
...
if ( ! function_exists( 'jws_login_ajax_callback' ) ) {
	function jws_login_ajax_callback() {
		...
		if ( empty( $_REQUEST['data'] ) ) {
			$response_data = array(
				'code'    => - 1,
				'message' => '<p class="jws-dealer-note red">' . esc_html__( 'Something wrong. Please try again.', 'meathouse' ) . '</p>'
			);
		} else {
			...
			if ( is_wp_error( $user_verify ) ) {
				if ( ! empty( $user_verify->errors ) ) {
					...
					$message = esc_html__( 'Something wrong. Please try again.', 'meathouse' );
				}
				$code = - 1;
			} else {
				$message = '<p class="jws-dealer-note green">' . esc_html__( 'Login successful, redirecting...', 'meathouse' ) . '</p>';
...
function my_user_register($user_id) {               // when a user registers, sends them an email to verify their account
    ...
    $html = ( '
            ...
            <h2 style="text-align: center;">Welcome to meathouse</h2>
...
* Function ajax filter
*/
if (!function_exists('jws_ajax_car_filter')) {
    function jws_ajax_car_filter()
    {
        $product_ex_ids = $product_inc_ids = $filter_categories = $filter_conditions = $posts_per_page = $orderby = $order = null;

        
        if(isset($_POST['product_ex_ids']) && !empty($_POST['product_ex_ids'])){
            $product_ex_ids = $_POST['product_ex_ids'];
        }
        if(isset($_POST['product_inc_ids']) && !empty($_POST['product_inc_ids'])){
            $product_inc_ids = $_POST['product_inc_ids'];
        }
        ...
        if (isset($_POST['filter_conditions'])) {
            $filter_conditions = sanitize_text_field($_POST['filter_conditions']);
        }
        ...
        if($_POST['layout'] == 'grid'){
            $class    = 'grid-layout row';
            $grid_class = 'car-item '.$_POST['display'].' col-xl-' . $_POST['columns'] . ' col-lg-' . $_POST['columns_tablet'] . ' col-' . $_POST['columns_mobile'] .'';
        }else {
             $grid_class = 'car-item '.$_POST['display'].'';
        }
        ...
        $wc_attr = array(
            'post_type' => 'cars',
            'car_cat' =>  $filter_categories,
            'car_condition' =>  $filter_conditions,
            'posts_per_page' => $posts_per_page,
            'paged' => $paged,
            'orderby' => $orderby,
            'order' => $order,
        ...
        <div class="products">
            ...
            $product_query->the_post();
             echo '<div class="'.esc_attr($grid_class).'">';    
             get_template_part( 'template-parts/content/cars/'.$_POST['display'].'' );
             echo '</div>';
    ...
    add_action('wp_ajax_jws_ajax_car_filter', 'jws_ajax_car_filter');
    add_action('wp_ajax_nopriv_jws_ajax_car_filter', 'jws_ajax_car_filter');
}
