...
class Jws_Logo extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Jws Logo', 'meathouse' );
	}
	...
	protected function register_controls() {
        ...
        $this->add_control(
			'image_sticky',
			[
				'label' => esc_html__( 'Choose Image Sticky', 'meathouse' ),
				'type' => Controls_Manager::MEDIA,
			]
		);
        $this->add_control(
			'image_special',
			[
				'label' => esc_html__( 'Choose Image Special (Display when change slider)', 'meathouse' ),
				'type' => Controls_Manager::MEDIA,
			]
		);
        $this->add_control(
			...
			[
				...
				'default' => esc_html__( 'full', 'meathouse' ),
				'placeholder' => esc_html__( 'Add size for image, example 300x300 or full,thumbnail', 'meathouse' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'file' => esc_html__( 'Media File', 'meathouse' ),
					'custom' => esc_html__( 'Custom URL', 'meathouse' ),
				],
			]
		);
        ...
        $this->add_control(
				'enable_sticky',
				[
					'label'        => esc_html__( 'Enable Sticky', 'meathouse' ),
					'type'         => Controls_Manager::SWITCHER,
					'label_on'     => esc_html__( 'Yes', 'meathouse' ),
					'label_off'    => esc_html__( 'No', 'meathouse' ),
					'return_value' => 'yes',
					'default'      => '',
					'description'  => esc_html__( 'Enable custom for sticky', 'meathouse' ),
        ...
        $this->add_responsive_control(
					...
					[
						...
						'size_units' 	=> [ 'px', 'em', '%' ],
						'selectors' 	=> [
							'.is-sticky  .logo_sticky_yes' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}',
						],
                        'condition'    => [
						  'enable_sticky' => 'yes',
					    ],
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'.is-sticky .logo_sticky_yes.elementor-element-{{ID}} .elementor-logo img' => 'width: {{SIZE}}{{UNIT}}',
				],
                'condition'    => [
						  'enable_sticky' => 'yes',
				],
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'.is-sticky .logo_sticky_yes.elementor-element-{{ID}} .elementor-logo img' => 'max-width: {{SIZE}}{{UNIT}}',
				],
                'condition'    => [
						  'enable_sticky' => 'yes',
				],	    
	...
	protected function render() {
        ...
        $id_logo = !empty($settings['image']['id']) ? $settings['image']['id'] : ''; 
		...
		<div <?php echo ''.$this->get_render_attribute_string( 'wrapper' ); ?>>
            ...
            <?php if(!empty($settings['image_special'])) : ?>
            <span class="logo-special">
                 <?php 
                     if (function_exists('jws_getImageBySize')) {
                         $attach_id = $settings['image_special']['id'];
                         $img = jws_getImageBySize(array('attach_id' => $attach_id, 'thumb_size' => $settings['logo_size'], 'class' => 'logo-image'));
                         echo !empty($img['thumbnail']) ? $img['thumbnail'] : '';
                     }
                ?>
