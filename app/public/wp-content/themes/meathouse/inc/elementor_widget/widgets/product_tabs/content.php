...
if($settings['layout'] == 'carousel') {
    ...
    if($settings['enble_muntirow'] == 'yes') {
       ...
       $data_slick = 'data-slick=\'{
        ...
        "centerMode":'.$center.',
        "infinite":'.$infinite.',
        "centerPadding":"'.$settings['center_mode_padding']['size'].'px",
        "arrows": '.$settings['show_nav'].', 
        "dots":'.$settings['show_pag'].',
        "speed": '.$settings['speed'].', 
        "responsive":[
            {"breakpoint": 1024,"settings":{"slidesToShow": '.$settings['slides_to_show_tablet'].',"slidesToScroll": '.$settings['scroll'].',"centerPadding":"'.$settings['center_mode_padding_tablet']['size'].'px"}},
        ...
        <div class="jws-head-filter has-tabs <?php echo esc_attr($settings['title'] ? 'has-border' : '');?>">
            <ul class="jws-ajax-load filter-asset ct_ul_ol">
              ...
              switch ( $settings['default_asset'] ) {
                 case 'featured':
                 $asset_title = esc_html__( 'Featured', 'meathouse' );
                 break;
                 case 'onsale':
                 $asset_title = esc_html__( 'On Sale', 'meathouse' );
                 break;
                 case 'deal':
                 $asset_title = esc_html__( 'Deal', 'meathouse' );
                 break;
                 case 'latest':
                 $asset_title = esc_html__( 'New Arrivals', 'meathouse' );
                 break;
                 case 'best-selling':
                 $asset_title = esc_html__( 'Best sellers', 'meathouse' );
                 break;
                 case 'toprate':
                 $asset_title = esc_html__( 'Top Rate', 'meathouse' );
                 break;
                 default:
                 break;
             }
            ...
            if ( $settings['filter_assets'] ) {
             ...
             foreach ( $settings['filter_assets'] as $val ) {
                switch ( $val ) {
                   case 'featured':
                   $html .= $settings['default_asset'] != 'featured' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="featured" title="' . esc_html__( 'Featured', 'meathouse' ) . '">' . esc_html__( 'Featured', 'meathouse' ) . '</a></li>' : '';
                   break;
                   case 'onsale':
                   $html .= $settings['default_asset'] != 'onsale' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="onsale" title="' . esc_html__( 'On Sale', 'meathouse' ) . '">' . esc_html__( 'On Sale', 'meathouse' ) . '</a></li>' : '';
                   break;
                   case 'deal':
                   $html .= $settings['default_asset'] != 'deal' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="deal" title="' . esc_html__( 'Deal', 'meathouse' ) . '">' . esc_html__( 'Deal', 'meathouse' ) . '</a></li>' : '';
                   break;
                   case 'latest':
                   $html .= $settings['default_asset'] != 'latest' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="latest" title="' . esc_html__( 'New Arrivals', 'meathouse' ) . '">' . esc_html__( 'New Arrivals', 'meathouse' ) . '</a></li>' : '';
                   break;
                   case 'best-selling':
                   $html .= $settings['default_asset'] != 'best-selling' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="best-selling" title="' . esc_html__( 'Best sellers', 'meathouse' ) . '">' . esc_html__( 'Best Seller', 'meathouse' ) . '</a></li>' : '';
                   break;
                   case 'toprate':
                   $html .= $settings['default_asset'] != 'toprate' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="toprate" title="' . esc_html__( 'Top Rate', 'meathouse' ) . '">' . esc_html__( 'Top Rate', 'meathouse' ) . '</a></li>' : '';
                   break;
                   default:
                   break;
               }
...
<?php if ( $product_query->max_num_pages > 1 && $settings['pagination'] == 'ajaxload') { ?>
    <div class="jws_product_pagination">
         <a class="jws-products-load-more" href="#">
            <span class="more_product"><?php echo esc_html__('Load More','meathouse'); ?></span>
         </a>
    </div>
<?php } ?>
