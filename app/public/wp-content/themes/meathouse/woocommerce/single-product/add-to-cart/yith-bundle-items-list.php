...
if ( $bundled_items ) : ?>
	<div class='yith-wcpb-product-bundled-items' <?php echo esc_attr( $style ); ?>>
		...
		foreach ( $bundled_items as $bundled_item ) :
			...
			$bundled_item_classes = apply_filters( 'yith_wcpb_bundled_item_classes', array( 'product', 'yith-wcpb-product-bundled-item product-item product' ), $bundled_item, $product );
			...
			<div class="<?php echo esc_attr( $bundled_item_classes ); ?>"
				...
				<div class="yith-wcpb-product-bundled-item-image">
					...
					if ( ! $hide_thumbnail ) {
						...
						if ( $post_thumbnail_id ) {
							...
							echo sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'meathouse' ) );
							echo '</div>';
						}
					}
				...
				<div class="yith-wcpb-product-bundled-item-data">
                    <div class="yith-wcpb-product-bundled-item-content">
					<div class="yith-wcpb-product-bundled-item-data__title">
						<?php if ( $bundled_product->is_visible() ) : ?>
							<a href="<?php echo esc_url( $bundled_product->get_permalink() ); ?>" class="<?php echo esc_attr( $quick_view_for_bundled_items ? 'yith-wcqv-button' : '' ); ?> woocommerce-loop-product__title" data-product_id="<?php echo esc_attr( $bundled_product->get_id() ); ?>">
							...
							<div class="yith-wcpb-bundled-quantity__invalid-notice">
								...
								echo wp_kses_post(
									...
									// translators: 1. the minimum amount; 2. the maximum amount.
										__( 'You can choose a minimum quantity of %1$s and a maximum quantity of %2$s for this product!', 'meathouse' ),
								...
								<?php foreach ( $b_attributes as $name => $options ) : ?>
									<tr>
										...
										<td class="value">
											...
											echo end( $attribute_keys ) === $name ? wp_kses_post( apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations" href="#">' . esc_html__( 'Clear', 'meathouse' ) . '</a>' ) ) : '';
											?>
										</td>
									</tr>
								<?php endforeach; ?>
