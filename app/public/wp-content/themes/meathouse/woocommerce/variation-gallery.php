...
if ( ! function_exists( 'jws_vg_admin_html' ) ) {
	function jws_vg_admin_html( $loop, $variation_data, $variation ) {
		...
		echo '<div class="jws-variation-gallery-wrapper">';
			echo '<h4>' . esc_html__( 'Variation Image Gallery', 'meathouse' ) . '</h4>';
			...
			echo '<a href="#" class="button jws-add-variation-gallery-image">' . esc_html__( 'Add Gallery Images', 'meathouse' ) . '</a>';
		echo '</div>';
	}
...
add_action( 'woocommerce_variation_options', 'jws_add_points_field_to_variations', 10, 3 );
 
function jws_add_points_field_to_variations( $loop, $variation_data, $variation ) {
    $id = get_the_ID();
    $default_value = get_post_meta( $variation->ID, '_box_points', true );
    ...
    for ($i = 1; $i <= 100; $i++) {
     ...
     woocommerce_wp_select( array(
        'id' => '_box_points[' . $loop . ']',
        'label'   => __( 'Choose points for product', 'meathouse' ),
...
add_action( 'woocommerce_save_product_variation', 'jws_save_points_field_variations', 10, 2 );
 
function jws_save_points_field_variations( $variation_id, $i ) {
  
   $custom_field = $_POST['_box_points'][$i];
   if ( isset( $custom_field ) ) update_post_meta( $variation_id, '_box_points', esc_attr( $custom_field ) );
...
add_filter( 'woocommerce_available_variation', 'jws_save_points_field_variation_data' );
 
function jws_save_points_field_variation_data( $variations ) {
   $variations['_box_points'] = get_post_meta( $variations[ 'variation_id' ], '_box_points', true );
   return $variations;
}