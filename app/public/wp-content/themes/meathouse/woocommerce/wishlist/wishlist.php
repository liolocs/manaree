...
if ( ! function_exists( 'jws_add_to_wishlist_btn' ) ) {
	...
	function jws_add_to_wishlist_btn() {
		global $product;
        $enable =  jws_theme_get_option('wishlist');
		...
		echo '<a href="' . esc_url( jws_get_wishlist_page_url() ) . '" class="jws-wishlist-btn'.esc_attr($class_added).'" data-id="' . esc_attr( $product->get_id() ) . '">
            <i class="jws-icon-glyph-16"></i>
            <span class="notadd">'.esc_html__('Add to wishlist','meathouse').'</span>
            <span class="added">'.esc_html__('Browse Wishlist','meathouse').'</span>
        </a>';
	}
}
...
if ( ! function_exists( 'jws_add_to_wishlist_single_btn' ) ) {
	...
	function jws_add_to_wishlist_single_btn() {
		global $product;
		echo '<a class="jws-wishlistsg-btn" data-added-text="' . esc_html__('Wishlist products', 'meathouse') . '" data-id="' . esc_attr( $product->get_id() ) . '"><span class="jws-icon-heart-straight-thin"></span></a>';
	}
}
			...
			if ( ! empty( $products ) ) {
					...
					?>
						...
						<table class="jws-wishlist-row">
                            <thead>
                    			<tr>
                    				...
                    				<th class="title_wishlist"><?php echo esc_html__('Product Detail','meathouse'); ?></th>
                                    <th class="title_wishlist"><?php echo esc_html__('Price','meathouse'); ?></th>
                    				<th class="title_wishlist"><?php echo esc_html__('Stock Status','meathouse'); ?></th>
                                    <th class="title_wishlist"></th>
                    			</tr>
                			</thead>
                            <tbody>
                        ...
                        <div class="add_all_product">
                            <?php 
                                ...
                                if(isset($jws_option['shop_layout']) && $jws_option['shop_layout'] != 'rent') { ?>
                                    <a href="<?php echo esc_url_raw( $add_to_cart_url); ?>" data-product_id="<?php echo esc_attr($product_ids); ?>"><?php esc_html_e('Add all to cart', 'meathouse'); ?></a>
                                <?php } ?>
                        </div>
					<?php
				}	
				...
				?>
					<h4 class="jws-empty-wishlist">
						<?php esc_html_e('Wishlist is empty.', 'meathouse'); ?>
					</h4>
					...
					<p class="return-to-shop">
						<a class="button" href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ); ?>">
							<?php esc_html_e( 'Return to shop', 'meathouse' ); ?>
						</a>
					</p>
				<?php
			}
...
if ( ! function_exists( 'jws_wishlist_availability_html' ) ) {
	...
	function jws_wishlist_availability_html( $product ) {
		...
		if( empty( $availability['availability'] ) ) {
			$availability['availability'] = esc_html__( 'In stock', 'meathouse' );
		}
