  <div class="product-item-inner deal">
    ...
    if(!empty($countdown_time)) { ?>
       <div class="jws-sale-time" data-d="<?php echo esc_attr__('DAYS','meathouse'); ?>" data-h="<?php echo esc_attr__('HRS','meathouse'); ?>" data-m="<?php echo esc_attr__('MIN','meathouse'); ?>" data-s="<?php echo esc_attr__('SEC','meathouse'); ?>" data-countdown="<?php echo esc_attr($countdown_time); ?>"></div> 
    <?php }?>
    <?php
      ...
      if(!empty($units_sold) && !empty($total)) {
        ...
        echo '<div class="progress-bar-sold">
                ...
                <span class="sold_count"><span>'.esc_html__('Sold:','meathouse').'</span><strong>'.$units_sold.'</strong></span>
                <span class="available_items"><span>'.esc_html__('Available:','meathouse').'</span><strong>'.$total.'</strong></span>
             </div>';
      }  
    ?>
   ...
   <a href="?add-to-cart=<?php echo get_the_ID(); ?>" data-quantity="1" class="btn-main elementor-button button product_type_simple add_to_cart_button ajax_add_to_cart" data-product_id="<?php echo get_the_ID(); ?>" data-product_sku=""  rel="nofollow" tabindex="-1"><span class="text">
    <?php 
        echo esc_html__('Add to cart','meathouse');
